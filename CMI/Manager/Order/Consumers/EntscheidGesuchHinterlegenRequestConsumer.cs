...
{
    public class EntscheidGesuchHinterlegenRequestConsumer : IConsumer<EntscheidGesuchHinterlegenRequest>
    {
        ...
        public async Task Consume(ConsumeContext<EntscheidGesuchHinterlegenRequest> context)
        {
            using (LogContext.PushProperty(nameof(context.ConversationId), context.ConversationId))
            {
                ...
                await manager.EntscheidGesuchHinterlegen(context.Message.UserId, context.Message.OrderItemIds, context.Message.Entscheid,
                    context.Message.DatumEntscheid, context.Message.InterneBemerkung);
                await context.RespondAsync(new EntscheidGesuchHinterlegenResponse());
            }
        }
    }
}