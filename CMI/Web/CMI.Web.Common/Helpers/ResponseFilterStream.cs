...
{
    // see http://weblog.west-wind.com/posts/2009/Nov/13/Capturing-and-Transforming-ASPNET-Output-with-ResponseFilter
    public class ResponseFilterStream : Stream
    {
        private const int DefaultMemoryStreamSize = 32 * 1024;
        ...
        {
            if (TransformString == null || HttpContext.Current == null)
            {
                return ms;
            }
        ...
        {
            if (IsCaptured && _cacheStream != null && _cacheStream.Length > 0)
            {
                try
                {
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            if (IsCaptured)
            {
                ...
                {
                    _cacheStream = new MemoryStream(DefaultMemoryStreamSize);
                }
        ...
        /// </summary>
        protected MemoryStream _cacheStream;

        /// <summary>
        ///     Internal pointer that that keeps track of the size
        ///     of the cacheStream
        /// </summary>
        protected int _cachePointer;
