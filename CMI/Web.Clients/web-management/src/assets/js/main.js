	...
	cmi.initJQForElement = function (elem, services) {
		...
		if ($(window).width() > 767) {
			...
			$elem.find('[type=date]').each(function () {
				...
				var _txt = services.translationService;
				var days = _txt.get('Calendar.Days.Short', 'So;Mo;Di;Mi;Do;Fr;Sa').split(';');
				var weekdays = _txt.get('Calendar.Days.Week', 'Sonntag;Montag;Dienstag;Mittwoch;Donnerstag;Freitag;Samstag;Sonntag').split(';');
				var months = _txt.get('Calendar.Months', 'Januar;Februar;März;April;Mai;Juni;Juli;August;September;Oktober;November;Dezember').split(';');
				...
				var picker = new Pikaday({
					...
					theme: 'admin-theme',
					i18n: {
						previousMonth: _txt.get('Calendar.Month.Previous', 'Vorheriger Monat'),
						nextMonth: _txt.get('Calendar.Month.Next', 'Nächster Monat'),
						months: months,
						weekdays: weekdays,
						weekdaysShort: days
					}
		...
		(function ($, data) {
			...
			var $searchFields = $elem.find('.form-search .search-field');
			if (data) {
			...
			$elem.find('.form-search').append('<button class="icon icon--search icon--before"></button>');
		...
		(function ($) {
			...
			// slideshow counter
			var slideshow_total = $elem.find('.carousel-slideshow .item').length;
			$('#carousel-total').text(slideshow_total);

			$elem.find('.carousel-slideshow').on('slid.bs.carousel', function () {
		...
		function carouselInit($) {
			...
			var $carousel = $elem.find('.carousel:not(.carousel-slideshow)');

			$elem.find('.carousel .item:first-child').addClass('first');
			$elem.find('.carousel .item:last-child').addClass('last');

			$elem.find('.carousel').each(function () {
				disableControl($(this));
			});
			$elem.find('.carousel').on('slid.bs.carousel', function () {
				disableControl($(this));
			});
		...
		(function ($) {
			...
			// Normal Collapse
			$elem.find('.collapse:not(tbody)').on('show.bs.collapse', function () {
				$(this)
					.prev()
			...
			$elem.find('.collapse:not(tbody)').on('hide.bs.collapse', function () {
				$(this)
					.prev()
			...
			$elem.find('tbody.collapse').on('show.bs.collapse', function () {
				$(this)
			...
			$elem.find('tbody.collapse').on('hide.bs.collapse', function () {
				$(this)
					.prev().find('[data-toggle=collapse]')
					.removeClass('active')
					.attr({
		...
		(function ($) {
			...
			$('.drilldown').not('.yamm-close-bottom').drilldown(options);
		...
		(function ($) {
			...
			var $yamm = $elem.find('.yamm'),
				$yammClose = $elem.find('.yamm-close, .yamm-close-bottom'),
				$dropdown = $elem.find('.yamm .dropdown'),
				$dropdownToggle = $elem.find('.yamm .dropdown-toggle');
		...
		(function ($) {
			...
			function carouselify() {
				var $tabFocus = $elem.find('.tab-focus'),
					focusIndex = 0;
			...
			function collapsify() {
				var $navTab = $elem.find('.nav-tabs:not(.focus)'),
					$collapsify = $elem.find('.collapsify'),
					linkIndex = 0;
		...
		(function ($) {
			...
			$elem.find('a[href=#collapseSubNav]').on('click', function () {
		...
		function subNavInit($) {
			...
			var $drilldown = $elem.find('.drilldown[class*=col-]');
		...
		(function ($) {
			...
			$elem.find('.table-sort').tablesorter();
		...
		(function ($) {
			...
			$elem.find('.treecrumb').each(function () {
				var $that = $(this);
