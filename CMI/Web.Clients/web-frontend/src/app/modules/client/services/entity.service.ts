...
export class EntityService {
	...
	public getAnonymized(id: string): Promise<unknown> {
		const apiDataUrl = this._options.serverUrl + this._options.publicPort + '/api/Data';
		const queryString = `?id=${id}&language=${this._context.language}`;

		const url = `${apiDataUrl}/GetAnonymized${queryString}`;
		return this._http.get<unknown>(url, this._http.noCaching).toPromise();
	}
