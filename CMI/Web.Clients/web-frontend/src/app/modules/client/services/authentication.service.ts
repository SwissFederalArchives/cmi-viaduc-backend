...
export class AuthenticationService {
	...
	public login(idp: string = null): void {
		...
		let returnUrl;
		// Wird man durch RequiresElevatedCheck ein zweites Mal mit QoA60 angemeldet
		// muss die returnUrl aus der zuvor gespeicherten Location geholt werden.
		// Ansonsten ist es die aktuelle Seite zu der man dann später zurück will.
		if (idp) {
			returnUrl = this._getUrl(authReturnUrlKey);
		} else {
	...
	private handleIdentityResponse(response: any): boolean {
		...
		switch (response.authStatus) {
			...
			case AuthStatus.requiresElevatedCheck:
				this.login('level-60');
				// Keine erfolgreiche Anmeldung. Ist notwendig, damit nicht
				// auf die ReturnUrl umgeleitet wird und diese dadurch gelöscht wird.
				// Dadurch ist die gepseicherte ReturnUrl noch vorhanden, wenn man sich
				// als BAR Mitarbeiter ein zweites Mal angemeldet hat.
				return false;
	...
	private _getUrl(key: string): string {
		const authMode = this._config.getSetting('authentication.mode');
		if (authMode === 'builtin') {
			return this._sessionStorage.getItem(key);
		} else {
			return this._sessionStorage.getUrl(key);
		}
	}
