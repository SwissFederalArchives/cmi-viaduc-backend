...
-- Spalten hinzufügen
ALTER TABLE [dbo].[Ordering] ADD [RolePublicClient] NVARCHAR(3) NULL
EXEC sys.sp_addextendedproperty @level1name=N'Ordering', @level2name=N'RolePublicClient', @value=N'Benutzerrolle des Bestellers zum Bestellzeitpunkt', @level0type=N'SCHEMA', @level0name=N'dbo', @level1type=N'TABLE', @level2type=N'COLUMN', @name=N'MS_Description'

GO
-- Update existing orders with the current role of the users. 
update ordering set rolePublicClient = (select rolePublicClient from ApplicationUser where id = userId)
